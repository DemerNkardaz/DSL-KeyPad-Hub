...
<template>
    ...
    <div class="xiangqi-side-buttons-container">
      ...
      <!-- Старт / Пауза -->
      <button
        class="xiangqi-btn xiangqi-side-btn"
        :title="t('xiangqi.start_warning')"
        @click="togglePause"
      >
        <img
          class="invert"
          :src="baseUrl + (isPaused ? 'images/flat_icons/flat_start.svg' : 'images/flat_icons/flat_pause.svg')"
        />
      </button>
      ...
      <button
        class="xiangqi-btn xiangqi-side-btn"
        :class="aiEnabled ? 'xiangqi-side-btn-allowed' : 'xiangqi-side-btn-restricted'"
        :title="t('xiangqi.ai_battleground')"
        @click="toggleAI"
      >
      ...
      -->
      <button
        class="xiangqi-btn xiangqi-side-btn xiangqi-piece-turn-wrapper"
        :class="turnWrapperClass"
        :title="`${t('xiangqi.turn')} ${gameState.currentTurn === 'r' ? t('xiangqi.red') : t('xiangqi.black')}`"
        disabled
      >
      ...
      <!-- Ссылка на Unicode -->
      <a
        class="xiangqi-btn xiangqi-side-btn xiangqi-side-btn-squared"
        href="https://home.unicode.org"
        target="_blank"
        :title="t('xiangqi.unicode')"
      >
        <img src="/images/flat_icons/unicode_logo.svg" />
      </a>

			<!-- Скрытие доски -->
			<button v-if="windowWidth <= 1050" class="xiangqi-btn xiangqi-side-btn transition-layer" @click="isBoardVisible = false">
				H
			</button>
